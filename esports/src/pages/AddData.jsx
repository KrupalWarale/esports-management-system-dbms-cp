...
function AddData() {
  ...
  const handleFinalTeamChange = async (e) => {
    ...
    try {
      ...
      // Show loading toast
      const loadingToast = toast.loading("Loading team players...");
      ...
      if (response.data && response.data.length > 0) {
        // Set available players for captain selection
        setCaptainSet(response.data);
        
        // Set player IDs for nicknames
        if (response.data.length >= 3) {
          setFinalForm(prevState => ({
            ...prevState,
            p1: response.data[0].pid,
            p2: response.data[1].pid,
            p3: response.data[2].pid
          }));
          ...
          setFinalForm(prevState => ({
            ...
            p1: response.data[0].pid,
            p2: response.data[1].pid,
            p3: null // Clear player 3 if not available
          }));
          toast.warning("Team has only 2 players. You may need to add more players.");
        } else if (response.data.length === 1) {
          setFinalForm(prevState => ({
            ...prevState,
            p1: response.data[0].pid,
            p2: null, // Clear player 2 if not available
            p3: null  // Clear player 3 if not available
          }));
          toast.warning("Team has only 1 player. You need to add more players.");
        }
        
        toast.success(`${response.data.length} players found for this team`);
      } else {
        toast.warning("No players found for this team");
        setCaptainSet([]);
        
        // Clear player IDs to prevent errors
        setFinalForm(prevState => ({
          ...prevState,
          p1: null,
          p2: null,
      ...
      toast.error("Error loading team players: " + (error.response?.data?.message || error.message));
      
      // Clear the players on error
      setCaptainSet([]);
      setFinalForm(prevState => ({
        ...prevState,
        p1: null,
  ...
  const handleMerchSubmit = async (e) => {
    ...
    if (!finalForm.team || !finalForm.captain) {
      toast.error("Team and captain selection are required");
      return;
    }
    
    // Make sure at least one nickname is set if we have players
    if (finalForm.p1 && !finalForm.nick1) {
      toast.error("Nickname for Player 1 is required");
      return;
    }
    
    try {
      // Set the team ID in merchDetails
      merchDetails.tid = finalForm.team;
      
      // Show loading indicator during API call
      const response = await toast.promise(
        axios.post("https://dbms-miniproject.onrender.com/add_captain_and_create_merch", {
          team: finalForm,
          merch: merchDetails
        }),
        {
          pending: "Assigning captain and setting up team...",
          success: "Team captain assigned successfully!",
      ...
      if (response.data && response.data.message === "Success") {
        ...
        // If the server response doesn't explicitly indicate success
        toast.warning("Captain was assigned but there might be some issues. Please verify in the dashboard.");
      }
    } catch (error) {
      console.error("Error in captain assignment:", error);
      toast.error(`Error: ${error.response?.data?.error || error.message || "Unknown error occurred"}`);
      setPopupStatus(false);
    }
  };
  return (
